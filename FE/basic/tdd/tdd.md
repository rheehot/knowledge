...
간단히 요약하자면 **`TDD`** 는 개발하려는 기능의 테스트케이스를 미리 작성을 하고 개발하는 방식 중 하나이다.

앞서 TDD방식의 단점으로 꼽힌 테스트케이스가 증가할수록 실제 개발 소스코드를 완성하기까지의 시간이 증가한다는 점은 프로그램의 전체 개발/유지보수를 고려한다면 오히려 TDD방식의 장점을 잘 나타내는 부분이라고 생각한다.
